...
func CreatePowerup(Which: int) ->  TPowerup:
	if (Which >= 0) && (Which < Constants.NUM_POWERUPS):
		...
		match Which:
			...
			Constants.POWERUP_CANPUNCH: powerup = TPunchPowerup.new()
			Constants.POWERUP_CANGRAB: powerup = TGrabPowerup.new()
			...
			Constants.POWERUP_JELLYBOMBS: powerup = TJellyPowerup.new()
